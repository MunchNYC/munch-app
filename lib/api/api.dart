...
abstract class Api {
  ...
  Api({String endpointSetPrefix, int version}) {
    baseUrl =
        backendBaseUrl + "/" + endpointSetPrefix + "/v" + version.toString();
  }
  ...
  Future<dynamic> performHttpRequest(
      ...
      Function(Map<String, String>) requestFunction}) async {
    if (headers == null) {
      headers = await generateHeaders();
    }

    for (int attempt = 0;
        attempt <= CommunicationSettings.numOfRetries;
        attempt++) {
      print("Headers: " + headers.toString());
      try {
        final response = await requestFunction(headers).timeout(
            Duration(seconds: CommunicationSettings.maxServerWaitTimeSec),
            onTimeout: () {
          throw ServerConnectionException.fromMessage(
              App.translate("api.error.request_timeout"));
        });
    ...
    switch (response.statusCode) {
      ...
      case 401:
        throw UnauthorisedException(response.statusCode,
            {"message": App.translate("api.error.unauthorized")});
      case 403:
...
class ServerConnectionException extends FetchDataException {
  ServerConnectionException.fromMessage(String message)
      : super.fromMessage(message);

  ServerConnectionException(int status, Map<String, dynamic> map)
      : super(status, map);
}

class InternetConnectionException extends FetchDataException {
  InternetConnectionException()
      : super.fromMessage(App.translate("api.error.internet_connection"));
}

class InternalServerErrorException extends FetchDataException {
  InternalServerErrorException.fromMessage(String message)
      : super.fromMessage(message);

  InternalServerErrorException(int status, Map<String, dynamic> map)
      : super(status, map);
}
