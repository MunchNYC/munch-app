...
class _MunchOptionsScreenState extends State<MunchOptionsScreen> {
        ...
        if (widget.munch.isModifiable)
          Padding(
              ...
              child: CustomButton<MunchState, MunchPreferencesSavingState>.bloc(
                ...
                content: Text(
                    App.translate("options_screen.app_bar.action.text"),
  ...
  void _kickMemberListener(KickingMemberState state) {
    Utility.showFlushbar(
        App.translate("options_screen.kick_member.successful"), context);
  }
  ...
  Widget _buildMunchBloc() {
    return BlocConsumer<MunchBloc, MunchState>(
        ...
        listener: (BuildContext context, MunchState state) =>
            _optionsScreenListener(context, state),
        ...
        builder: (BuildContext context, MunchState state) =>
            _buildOptionsScreen(context, state));
  }
        ...
        child: Form(
            ...
            child: Column(
                ...
                children: [
                  ...
                  if (widget.munch.isModifiable &&
                      UserRepo.getInstance().currentUser.uid ==
                          widget.munch.hostUserId)
                    _changeLocationRow(),
                  Divider(
                      height: 36.0,
                      thickness: 1.0,
      ...
      children: <Widget>[
        Expanded(
            child: CustomFormField(
                labelText:
                    App.translate("options_screen.munch_name_field.label.text"),
        ...
        if (widget.munch.isModifiable)
          CustomButton(
              ...
              content: _munchNameFieldReadOnly
                  ? Text(
                      App.translate(
                          "options_screen.munch_name_field.readonly_state.text"),
                      style: AppTextStyle.style(AppTextStylePattern.heading6,
                          fontWeight: FontWeight.w400,
                          color: Palette.hyperlink))
                  : FaIcon(FontAwesomeIcons.solidTimesCircle,
                      size: 14.0, color: Palette.secondaryLight.withAlpha(150)),
              onPressedCallback: () {
    ...
    return Row(
      ...
      children: <Widget>[
        Expanded(
            child: CustomFormField(
          labelText:
              App.translate("options_screen.munch_link_field.label.text"),
          labelStyle: AppTextStyle.style(AppTextStylePattern.heading6,
              fontWeight: FontWeight.w500,
              color: Palette.primary.withOpacity(0.7)),
          textStyle: AppTextStyle.style(AppTextStylePattern.heading6,
        ...
        if (widget.munch.isModifiable)
          CustomButton(
            ...
            content: Text(
                App.translate(
                    "options_screen.invite_friends.share_action.text"),
                style: AppTextStyle.style(AppTextStylePattern.heading6,
      ...
      children: <Widget>[
        Expanded(
          ...
          labelText: App.translate(
              "options_screen.push_notifications_field.label.text"),
          labelStyle: AppTextStyle.style(AppTextStylePattern.heading6,
              fontWeight: FontWeight.w500,
              color: Palette.primary.withOpacity(0.7)),
          textStyle: AppTextStyle.style(AppTextStylePattern.heading6,
          ...
          borderRadius: 0.0,
          borderColor: Palette.background,
          initialValue: App.translate(
              "options_screen.push_notifications_field.value.text"),
          readOnly: true,
          maxLines: 2,
        )),
        SizedBox(width: 12.0),
        CupertinoSwitch(
            ...
            onChanged: widget.munch.isModifiable
                ? _onPushNotificationsSwitchChanged
                : null),
      ],
    );
  }
      ...
      Row(
        ...
        children: <Widget>[
          Expanded(
              child: CustomFormField(
            labelText: App.translate(
                "options_screen.change_location_field.label.text"),
            labelStyle: AppTextStyle.style(AppTextStylePattern.heading6,
                fontWeight: FontWeight.w500,
                color: Palette.primary.withOpacity(0.7)),
            textStyle: AppTextStyle.style(AppTextStylePattern.heading6,
            ...
            borderColor: Palette.background,
            initialValue: App.translate(
                "options_screen.change_location_field.value.text"),
            readOnly: true,
            maxLines: 1,
          )),
          SizedBox(width: 12.0),
          CustomButton(
            ...
            color: Colors.transparent,
            content: Text(
                App.translate("options_screen.change_location_button.text"),
                style: AppTextStyle.style(AppTextStylePattern.heading6,
                    fontWeight: FontWeight.w400, color: Palette.hyperlink)),
  ...
  Widget _membersListTrailing(User user) {
    if (user.uid == widget.munch.hostUserId) {
      return Text(App.translate("options_screen.member_list.host.text"),
          style: AppTextStyle.style(AppTextStylePattern.heading6,
              fontWeight: FontWeight.w500, color: Palette.primary));
    } else if (widget.munch.isModifiable &&
        widget.munch.hostUserId == UserRepo.getInstance().currentUser.uid) {
      return CustomButton<MunchState, KickingMemberState>.bloc(
          ...
          content: Text(
              App.translate("options_screen.member_list.kick_button.text"),
              style: AppTextStyle.style(AppTextStylePattern.heading6,
  ...
  String _validateMunchName(String munchName) {
    if (munchName.trim().isEmpty) {
      return App.translate(
          "options_screen.preferences_form.name_field.required.validation");
    }
    ...
    if (!regex.hasMatch(munchName)) {
      return App.translate(
          "options_screen.preferences_form.name_field.regex.validation");
    } else {
      return null;
    }
  }
  ...
  void _onMunchLinkClicked() {
    ...
    Utility.showFlushbar(
        App.translate("options_screen.copy_action.successful"), context,
  ...
  void _onShareButtonClicked() async {
    await WcFlutterShare.share(
        sharePopupTitle:
            App.translate("options_screen.share_button.popup.title"),
  ...
  void _onKickButtonClicked(User user) {
    OverlayDialogHelper(
            widget: KickMemberAlertDialog(
                user: user, munchId: widget.munch.id, munchBloc: _munchBloc))
        .show(context);
  }

  void _onUpdateLocationButtonClicked() {
    NavigationHelper.navigateToMapScreen(context,
            editLocation: true, munch: widget.munch)
