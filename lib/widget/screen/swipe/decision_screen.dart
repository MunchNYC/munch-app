...
class _DecisionScreenState extends State<DecisionScreen> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        child: Scaffold(
            ...
            appBar:
                AppStatusBar.getAppStatusBar(iconBrightness: Brightness.light),
  ...
  void _decisionScreenListener(BuildContext context, MunchState state) {
    if (state.hasError) {
      ...
      Utility.showFlushbar(
          App.translate("decision_screen.review_munch.successful.text"),
          context);
    }
  }
  ...
  Widget _buildMunchBloc() {
    return BlocConsumer<MunchBloc, MunchState>(
        ...
        listenWhen: (MunchState previous, MunchState current) =>
            current.hasError ||
            current.ready ||
            (current.loading && current is ReviewMunchState),
        listener: (BuildContext context, MunchState state) =>
            _decisionScreenListener(context, state),
        ...
        builder: (BuildContext context, MunchState state) =>
            _buildDecisionScreen(context, state));
  }

  Widget _buildDecisionScreen(BuildContext context, MunchState state) {
    if (state.hasError) {
      return ErrorPageWidget();
    } else if ((state.initial && widget.shouldFetchDetailedMunch) ||
        (state.loading && state is DetailedMunchFetchingState) ||
        (state.ready && widget.restaurant == null)) {
            ...
            children: [
              ...
              Padding(
                  ...
                  child: Column(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: <Widget>[
                      if (!widget.munch.isModifiable) _unmodifiableInfoRow(),
                      if (!widget.munch.isModifiable)
                        Divider(
                            thickness: 2.0,
                            height: 32.0,
                            color: Palette.secondaryLight.withOpacity(0.2)),
                      _restaurantDetails(),
                      if (widget.munch.isModifiable) SizedBox(height: 16.0),
                      if (widget.munch.isModifiable) _buttonControls()
                    ],
                  ))
            ]));
  }
          ...
          children: <Widget>[
            ...
            Padding(
                ...
                child: Row(
                  ...
                  children: <Widget>[_closeIcon(), _moreIcon()],
                )),
            Positioned(
              right: 12.0,
              bottom: 4.0,
              child: Text(
                (_currentCarouselPage + 1).toString() +
                    "/" +
                    widget.restaurant.photoUrls.length.toString(),
      ...
      Expanded(
        child: Column(
          ...
          children: [
            Text(
                App.translate(
                        "decision_screen.munch_unmodifiable.description") +
                    " " +
            ...
            CustomButton(
              ...
              content: Text(
                  App.translate(
                      "decision_screen.munch_unmodifiable.feedback_button.text"),
              ...
              onPressedCallback: () {
                ...
                OverlayDialogHelper(
                        widget: ReviewMunchDialog(
                            munchBloc: _munchBloc, munch: widget.munch))
                    .show(context);
              },
            )
          ],
  ...
  Widget _restaurantDetails() {
    return Column(
      ...
      children: <Widget>[
        ...
        SizedBox(height: 8.0),
        Text(
            (widget.restaurant.priceSymbol != null
                    ? widget.restaurant.priceSymbol + ' • '
                    : "") +
        ...
        if (widget.restaurant.workingHours != null &&
            widget.restaurant.workingHours.length > 0)
          _workingHours(),
      ],
    );
  }
  ...
  Widget _workingHours() {
    return Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Text(App.translate("decision_screen.restaurant.hours.subtitle"),
            style: AppTextStyle.style(AppTextStylePattern.body2,
                color: Palette.primary.withOpacity(0.7),
          ...
          Row(
            ...
            children: <Widget>[
              SizedBox(
                  width: 100.0,
                  child: Text(
                      "${workingHours.dayOfWeek[0]}${workingHours.dayOfWeek.substring(1).toLowerCase()}",
                      style: AppTextStyle.style(AppTextStylePattern.body2,
                          color: Palette.primary.withOpacity(0.6)))),
              Flexible(
                  child: Text(workingHours.getWorkingTimesFormatted(),
                      maxLines: 2,
                      style: AppTextStyle.style(AppTextStylePattern.body2,
                          color: Palette.primary.withOpacity(0.6)))),
            ],
    ...
    return Column(
      ...
      children: <Widget>[
        CustomButton(
          ...
          onPressedCallback: () {
            Utility.launchUrl(
                context, "tel://" + widget.restaurant.phoneNumber);
          },
        ),
  ...
  Widget _shareButton() {
    return Column(
        ...
        children: <Widget>[
          CustomButton(
            ...
            onPressedCallback: () async {
              await WcFlutterShare.share(
                  sharePopupTitle:
                      App.translate("decision_screen.share_button.popup.title"),
          ...
          Text(App.translate("decision_screen.share_button.label.text"),
              style: AppTextStyle.style(AppTextStylePattern.body2,
                  color: Palette.primary)),
        ]);
  }
  ...
  Widget _yelpStatsRow() {
    return GestureDetector(
        ...
        child: Row(
          ...
          children: <Widget>[
            Image(
                image: AssetImage("assets/images/yelp/yelp-burst.png"),
                height: 16.0),
            SizedBox(width: 8.0),
            _starsRating(),
            SizedBox(width: 8.0),
            Text(
                widget.restaurant.reviewsNumber.toString() +
                    " " +
                    App.translate('decision_screen.yelp.reviews.text'),
                style: AppTextStyle.style(AppTextStylePattern.body2,
  ...
  Widget _starsRating() {
    String afterDecimalPointValue = widget.restaurant.rating
        .toString()
        .substring(widget.restaurant.rating.toString().indexOf(".") + 1);
    return Image(
    ...
    return Row(
      children: <Widget>[
        Expanded(
          ...
          content: Text(
              App.translate("decision_screen.continue_exploring_button.text"),
              style:
                  AppTextStyle.style(AppTextStylePattern.body2SecondaryDark)),
        ...
        Expanded(
          ...
          padding: EdgeInsets.symmetric(vertical: 12.0),
          content: Text(
              App.translate("decision_screen.new_restaurant_button.text"),
              style:
                  AppTextStyle.style(AppTextStylePattern.body2SecondaryDark)),
          onPressedCallback: () {
            OverlayDialogHelper(
                    widget: NewRestaurantAlertDialog(
                        munchId: widget.munch.id, munchBloc: _munchBloc))
                .show(context);
          },
        )),
      ],
