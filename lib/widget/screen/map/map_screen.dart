...
class MapScreenState extends State<MapScreen> {
  ...
  GoogleMapsPlaces _googleMapsPlaces =
      GoogleMapsPlaces(apiKey: AppConfig.getInstance().googleMapsApiKey);
  ...
  static List<int> _radiusValuesMetres = RADIUS_VALUES_MILES
      .map((value) => (value * MILES_TO_METERS_RATIO).floor())
      .toList();
  ...
  Widget _floatingActionButton() {
    return FloatingActionButton(
        backgroundColor: Palette.background,
        child: ImageIcon(AssetImage("assets/icons/locateMe.png"),
            size: 20.0, color: Palette.hyperlink),
        onPressed: () {
  ...
  void _initCurrentMunchLocation() {
    _initialCameraPosition = LatLng(
        widget.munch.coordinates.latitude, widget.munch.coordinates.longitude);
    ...
    return new Scaffold(
      ...
      body: BlocConsumer<LocationBloc, LocationState>(
          ...
          listener: (BuildContext context, LocationState state) =>
              _locationListener(context, state),
          buildWhen: (LocationState previous, LocationState current) =>
              current is CurrentLocationFetchingState,
          builder: (BuildContext context, LocationState state) =>
              _buildMapScreen(context, state)),
    );
  }
  ...
  Widget _googleMap(BuildContext context) {
    return GoogleMap(
      ...
      initialCameraPosition:
          CameraPosition(target: _initialCameraPosition, zoom: _initialMapZoom),
  ...
  Widget _searchBar(BuildContext context) {
    return Material(
        ...
        child: CustomFormField(
            textStyle: AppTextStyle.style(AppTextStylePattern.body2,
                color: Palette.primary, fontSizeOffset: 1.0),
            hintText: App.translate("map_screen.search_field.placeholder.text"),
            hintStyle: AppTextStyle.style(AppTextStylePattern.body2,
                color: Palette.secondaryLight, fontSizeOffset: 1.0),
  ...
  Widget _buildRadiusSelectionButton(int index) {
    return CustomButton(
      minWidth: 64.0,
      borderRadius: 4.0,
      color: _circleRadius == _radiusValuesMetres[index]
          ? Palette.secondaryDark
          : Palette.background,
      content: Text(
          RADIUS_VALUES_MILES[index].toString() +
              " " +
              App.translate("map_screen.distance_button.unit.text"),
      ...
      onPressedCallback: () {
        setState(() {
          ...
          _centralCircle =
              _centralCircle.copyWith(radiusParam: _circleRadius.toDouble());
        });
      },
    );
  }
        ...
        */
        ...
        child: Center(
            child: Icon(Icons.location_on,
                color: Palette.ternaryDark, size: 44.0)));
  }
  ...
  void _submitButtonListener(BuildContext context, MunchState state) {
    if (state.hasError) {
      ...
      if (state is MunchCreatingState) {
        ...
        DialogHelper(
                dialogContent: MunchCodeDialog(createdMunch), isModal: true)
            .show(context);
      }
    }
  }
  ...
  Widget _buildSubmitButtonListener() {
    return BlocListener<MunchBloc, MunchState>(
      ...
      listener: (BuildContext context, MunchState state) =>
          _submitButtonListener(context, state),
    ...
    return CustomButton<MunchState, MunchCreatingState>.bloc(
      ...
      content: Text(App.translate("map_screen.lets_eat_button.text"),
          style: AppTextStyle.style(AppTextStylePattern.body3Inverse,
              fontWeight: FontWeight.w600, fontSizeOffset: 1.0)),
      onPressedCallback: () {
  ...
  void _onLetsEatButtonClicked() {
    Munch munch = Munch(
        ...
        coordinates: Coordinates(
            latitude: _centralCircle.center.latitude,
            longitude: _centralCircle.center.longitude),
  ...
  Widget _saveLocationButton() {
    return CustomButton(
      ...
      onPressedCallback: () {
        ...
        if (!widget.munch.coordinates.equals(newCoordinates) ||
            widget.munch.radius != _circleRadius) {
          munchWithUpdatedLocation =
              Munch(coordinates: newCoordinates, radius: _circleRadius);
        }

        return NavigationHelper.popRoute(context,
            result: munchWithUpdatedLocation);
      },
    );
  }
  ...
  Future _onSearchBarClicked() async {
    ...
    if (prediction != null) {
      PlacesDetailsResponse detail =
          await _googleMapsPlaces.getDetailsByPlaceId(prediction.placeId);
