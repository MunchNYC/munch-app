...
class MunchesTabState extends State<MunchesTab> {
  ...
  void _checkMunchCacheRefresh() {
    if (_munchRepo.getMunchesCallLastUTC != null) {
      Duration differenceLastFetch =
          DateTime.now().toUtc().difference(_munchRepo.getMunchesCallLastUTC);
                    ...
                    if (_showDecidedNotification)
                      Padding(
                          padding: EdgeInsets.only(
                              bottom: AppDimensions.scaleSizeToScreen(16.0)),
                          child: Icon(Icons.circle,
                              color: Color(0xFFE60000),
                              size: AppDimensions.scaleSizeToScreen(10.0)))
                  ],
                )),
              ],
            )));
  ...
  void _openReviewMunchDialog(
      {Munch munch, String imageUrl, bool forcedReview = false}) {
    OverlayDialogHelper(
  ...
  void _getMunchesListener(GetMunchesResponse getMunchesResponse) {
    ...
    _unmodifiableMunches =
        _munchRepo.munchStatusLists[MunchStatus.UNMODIFIABLE];
    ...
    if (_decidedMunches.length + _unmodifiableMunches.length <
        INITIAL_THRESHOLD_FOR_HISTORICAL_MUNCHES) {
  ...
  Widget _tabsContent() {
    return BlocConsumer<MunchBloc, MunchState>(
        ...
        listener: (BuildContext context, MunchState state) =>
            _listViewsListener(context, state),
        buildWhen: (MunchState previous, MunchState current) =>
            ...
            (current is MunchJoiningState ||
                current is ReviewMunchState && current.ready),
        builder: (BuildContext context, MunchState state) =>
            _buildListViews(context, state));
  }
  ...
  Widget _buildListViews(BuildContext context, MunchState state) {
    if (state.hasError) {
      return _renderListViews(errorOccurred: true);
        ...
        !(state is HistoricalMunchesPageFetchingState)) {
  ...
  Widget _renderListViews({bool errorOccurred = false, bool loading = false}) {
    ...
    return IndexedStack(index: _currentTab, children: <Widget>[
      _renderStillDecidingTabContent(
          errorOccurred: errorOccurred, loading: loading),
  ...
  Widget _renderStillDecidingListWidget(bool loading) {
    return RefreshIndicator(
        ...
        child: ListView.separated(
          ...
          itemCount:
              loading ? SKELETON_ITEMS_NUMBER : _stillDecidingMunches.length,
  ...
  Widget _renderStillDecidingTabContent(
      {bool errorOccurred = false, bool loading = false}) {
    if (errorOccurred) {
    ...
    return InkWell(
        onTap: () {
          NavigationHelper.navigateToDecisionScreen(context,
              munch: munch, shouldFetchDetailedMunch: true);
        },
        child: Slidable(
            ...
            IconSlideAction(
                ...
                iconWidget: Column(
                  ...
                  children: [
                    Expanded(
                        child: ImageIcon(
                            AssetImage("assets/icons/leaveReview.png"),
  ...
  bool _decidedListViewScrollNotificationHandler(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ListView.separated(
                    ...
                    itemCount: loading
                        ...
                        : _decidedMunches.length +
                            _unmodifiableMunches.length +
                            _historicalMunches.length,
                    padding: EdgeInsets.zero,
                    separatorBuilder: (BuildContext context, int index) {
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      if (index < _decidedMunches.length) {
                        munch = _decidedMunches[index];
                          ...
                          _decidedMunches.length +
                              _unmodifiableMunches.length) {
                        munch = _unmodifiableMunches[
                            index - _decidedMunches.length];
                      } else {
                        munch = _historicalMunches[index -
                            _decidedMunches.length -
                            _unmodifiableMunches.length];
                      }
   ...
   */
  Widget _renderDecidedTabContent(
      {bool errorOccurred = false, bool loading = false}) {
    if (errorOccurred) {
      return _renderErrorMunchesListWidget();
    } else if (loading ||
  ...
  void _onPlusButtonClicked(BuildContext context) {
    DialogHelper(
            dialogContent: CreateJoinDialog(munchBloc: munchBloc),
            rootNavigator: true)
        .show(context);
  }
