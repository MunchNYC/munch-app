...
class MunchListWidgetSkeleton extends StatelessWidget {
  ...
  List<List<double>> _skeletonScreenWidthPercentages = [
    [0.5, 0.2, 0.3],
    // munch name skeleton screen width %, munch members screen width %, munch status screen width %
    [0.4, 0.2, 0.3],
    [0.6, 0.3, 0.4]
  ];
  ...
  Widget _munchLeading() {
    return AspectRatio(
      aspectRatio: 1,
      child: CircleAvatar(
          backgroundColor: Palette.secondaryLight.withOpacity(0.5)),
    );
  }
  ...
  Widget _munchInfo() {
    return Column(
        ...
        children: <Widget>[
          Container(
            width:
                App.screenWidth * _skeletonScreenWidthPercentages[index % 3][0],
            height: AppDimensions.scaleSizeToScreen(20.0),
            color: Palette.secondaryLight.withOpacity(0.5),
          ),
          Container(
            width:
                App.screenWidth * _skeletonScreenWidthPercentages[index % 3][1],
            height: AppDimensions.scaleSizeToScreen(16.0),
            color: Palette.secondaryLight.withOpacity(0.5),
          ),
          Container(
            width:
                App.screenWidth * _skeletonScreenWidthPercentages[index % 3][2],
            height: AppDimensions.scaleSizeToScreen(16.0),
            color: Palette.secondaryLight.withOpacity(0.5),
          )
        ]);
