...
class ReviewMunchDialog extends StatelessWidget {
  ...
  ReviewMunchDialog(
      {this.munchBloc, this.munch, this.imageUrl, this.forcedReview = false});
    ...
    return CustomButton(
      ...
      onPressedCallback: () {
        munchBloc.add(ReviewMunchEvent(
            munchReviewValue: MunchReviewValue.LIKED,
            munchId: munch.id,
  ...
  Widget _neutralButton(MunchState state) {
    return CustomButton(
      ...
      onPressedCallback: () {
        munchBloc.add(ReviewMunchEvent(
            munchReviewValue: MunchReviewValue.NEUTRAL,
            munchId: munch.id,
            forcedReview: forcedReview));
      },
    );
  ...
  Widget _dislikeButton(MunchState state) {
    return CustomButton(
      ...
      onPressedCallback: () {
        munchBloc.add(ReviewMunchEvent(
            munchReviewValue: MunchReviewValue.DISLIKED,
            munchId: munch.id,
            forcedReview: forcedReview));
      },
    );
  ...
  Widget _didNotGoLabel(MunchState state) {
    return CustomButton(
      ...
      content: Text(App.translate("review_munch_dialog.did_not_go_label.text"),
          style: AppTextStyle.style(AppTextStylePattern.heading6SecondaryDark,
              fontWeight: FontWeight.w500),
          textAlign: TextAlign.center),
      onPressedCallback: () {
        munchBloc.add(ReviewMunchEvent(
            munchReviewValue: MunchReviewValue.NOSHOW,
            munchId: munch.id,
            forcedReview: forcedReview));
      },
    );
    ...
    return CustomButton(
      ...
      content: Text(App.translate("review_munch_dialog.skip_label.text"),
          style: AppTextStyle.style(AppTextStylePattern.body2,
              fontSizeOffset: 2.0, color: Palette.secondaryLight),
          textAlign: TextAlign.center),
      onPressedCallback: () {
        if (forcedReview) {
          munchBloc.add(ReviewMunchEvent(
              munchReviewValue: MunchReviewValue.SKIPPED,
              munchId: munch.id,
              forcedReview: forcedReview));
        } else {
