...
class NotificationsBloc extends Bloc<NotificationsEvent, NotificationsState> {
  ...
  @override
  void onTransition(
      Transition<NotificationsEvent, NotificationsState> transition) {
  ...
  Stream<NotificationsState> _getDetailedMunch(
      MunchDataChangedNotificationsEvent event) async* {
    ...
    try {
      ...
      print("Munch fetching failed from foreground notification: " +
          error.toString());
      yield DetailedMunchNotificationState.failed(
          exception: error, message: error.toString());
    }
  }

  Stream<NotificationsState> _currentUserKickedFromMunch(
      KickMemberNotificationEvent event) async* {
    String munchId = event.munchId;
