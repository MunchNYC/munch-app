...
class MunchRepo {
  ...
  Future<List<Munch>> getHistoricalMunchesNextPage() async {
    ...
    List<Munch> historicalMunchesList = await _munchApi.getHistoricalMunches(
        page: _historicalPageNumber, timestamp: _historicalPaginationUTCDate.millisecondsSinceEpoch);
  ...
  Future<Munch> swipeRestaurant({String munchId, String restaurantId, bool liked}) async {
    try {
      Munch munch = await _munchApi.swipeRestaurant(munchId: munchId, restaurantId: restaurantId, liked: liked);
  ...
  Future<Munch> kickMember({String munchId, String userId}) async {
    try {
      Munch munch = await _munchApi.removeUserFromMunch(munchId: munchId, userId: userId);
